...
class MeTRCP(nn.Module):
    def __init__(self, param, tokenizer):
        ...
        self.cross_attention_layer = CrossAttention(self.d_model, num_heads=128)
        # co-attention版：之前觉得隐藏层设得不够多，现在改成self.d_model*4
...
#                                             batch_size=64)
        # cross-attention版：
        self.fc_catalyst1 = FullyConnectedNN(input_dim=self.d_model, num_classes=self.num_classes[0], hidden_dim=self.d_model*2,
                                             batch_size=64)
        self.fc_solvent1 = FullyConnectedNN(input_dim=self.d_model, num_classes=self.num_classes[1], hidden_dim=self.d_model*2,
                                             batch_size=64)
        self.fc_solvent2 = FullyConnectedNN(input_dim=self.d_model, num_classes=self.num_classes[2], hidden_dim=self.d_model*2,
                                            batch_size=64)
        self.fc_reagent1 = FullyConnectedNN(input_dim=self.d_model, num_classes=self.num_classes[3], hidden_dim=self.d_model*2,
                                            batch_size=64)
        self.fc_reagent2 = FullyConnectedNN(input_dim=self.d_model, num_classes=self.num_classes[4], hidden_dim=self.d_model*2,
                                            batch_size=64)
